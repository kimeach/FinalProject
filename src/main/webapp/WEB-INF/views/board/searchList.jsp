...
<head>
  ...
  <title>게시글 목록</title>
	...
	function fn_search_form(url, keyword){
	     ...
	     //searchInput.setAttribute("name","searchOption");
	    // searchInput.setAttribute("value", searchOption);
	      ...
	      <td colspan="5" align="center"> 
		  	<div class="pagination">
		  				<c:if test="${pageMaker.prev==true}">
		  					<a href="${contextPath}/board/listPaging.do?page=${pageMaker.startPage-1}">&laquo;</a>		  					
		  				</c:if>
		  				
		  				<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
		  					<span <c:out value="${pageMaker.criteria.page==idx?'class=active':''}"/>>
		  						<a href="${contextPath}/board/listPaging.do?page=${idx}">${idx}</a>		
		  					</span>   					
		  				</c:forEach>
		  				
		  				<c:if test="${pageMaker.next==true && not empty pageMaker.endPage}">		  				
		  						<a href="${contextPath}/board/listPaging.do?page=${pageMaker.endPage+1}">&raquo;</a> 
		  				</c:if>
		  	</div>	
		  </td>
     </tr>
 	...
 	<c:when test="${articlesList !=null }" >
    	<c:forEach  var="article" items="${articlesList }" varStatus="articleNum" >
	    	<tr align="center">
				<td width="5%">${article.articleNO}</td>
				<td width="7%">${article.id }</td>
				<td width="12%">[${article.place1}][${article.place2}]</td>
				<td align='left'  width="30%">
				  <span style="padding-right:30px"></span>
				   <c:choose>
